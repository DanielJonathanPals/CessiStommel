...
- `noise_rate_prototype::Matrix{Float64}`: Is some Matrix with the same dimensions as the `du` 
    ...
    for corresponding instances of the PhaseDynamics and PhaseDynamics strucs.
...
"""
function deterministic_phase_dynamics!(du, u, t_0, t)
    du[1] = rhs_deterministic(u)
end
...
"""
function stochastic_phase_dynamics!(du, u, t_0, t)
    du[1] = rhs_stochastic(u)
end
...
"""
DeterministicPhaseDynamics = PhaseDynamics(deterministic_phase_dynamics!,
                                            stochastic_phase_dynamics!)
